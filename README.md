...
*   **Parquet** (`format: parquet`, `parquet` feature, included in `full`): each batch becomes one `.parquet` object whose schema is inferred from that batch, so DataFusion, DuckDB and other lake engines can query the prefix directly. Every payload must be a JSON object; any other payload fails on its own (to the DLQ) and the rest of the batch is still written. On a Parquet object, `compression` picks the column codec (`gzip`, `lz4` = LZ4_RAW, `zstd`) instead of wrapping the whole file, so the extension stays `.parquet`. As a source it emits one JSON-object message per row, with typed values, and it also reads snappy-compressed files written by other tools. The `file` endpoint rejects `parquet` because a Parquet file can't be appended to.
