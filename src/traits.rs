...
pub type CommitFunc =
    Box<dyn FnOnce(Option<CanonicalMessage>) -> BoxFuture<'static, anyhow::Result<()>> + Send + 'static>;

/// A closure for committing a batch of messages.
pub type BatchCommitFunc =
    Box<dyn FnOnce(Option<Vec<CanonicalMessage>>) -> BoxFuture<'static, anyhow::Result<()>> + Send + 'static>;
